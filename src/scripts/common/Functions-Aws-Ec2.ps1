...
function Wait-Ec2InstanceReady
{
    ...
    while ($true)
    {
        ...
        if ($instanceStatus.Status.Details | All -Predicate { $_.Status -eq "passed" })
        {
...
function New-AwsEc2Instance
{
    ...
    try
    {
        ...
        $augmentedInstanceParams = @{
            "AccessKey"="$awsKey";
            "SecretKey"="$awsSecret";
            "Region"="$awsRegion";
        }

        foreach ($key in $instanceParameters.Keys)
        {
            $augmentedInstanceParams.Add($key, $instanceParameters[$key])
        }

        $instanceRequest = New-EC2Instance @augmentedInstanceParams
        ...
        # Sometimes if you try to query the fresh instance immediately after it has just been created you get nothing. Lets add a sleep to avoid that.
        Sleep -Seconds 2
...
function New-Ec2InstanceForEdit
{
    ...
    try
    {
        ...
        if (-not $unattended)
        {
            if ([string]::IsNullOrEmpty($remoteUsername) -or [string]::IsNullOrEmpty($remotePassword))
            {
                Write-Verbose "One of remote username/password was empty. No remote connection to the instance will be opened."
            }
            else
            {
        ...
        New-AwsEc2Image -AwsKey $awsKey -AwsSecret $awsSecret -AwsRegion $awsRegion -Instance $instance -Name $imageName -comment $imageComment -unattended:$unattended
...
function New-AwsEc2Image
{
    ...
    if ($snapshotAnswer -eq "snapshot")
    {
        ...
        if ([string]::IsNullOrEmpty($name))
        {
            ...
            $name = Get-IncrementedVersionedImageName $imageName 
        }
        ...
        if ([string]::IsNullOrEmpty($comment))
        {
            ...
            $user = whoami
            $comment = StringNullOrEmptyCoalesce $userDescription "No information entered by user [$user] when creating image."
        }

        $result = New-Ec2Image -InstanceId $instance.InstanceId -Name "$name" -Description "$comment" -AccessKey $awsKey -SecretKey $awsSecret -Region $awsRegion
